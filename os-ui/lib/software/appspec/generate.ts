...
export function buildGeneratePrompt(m: GenerateMaterial): GeneratePrompt {
  const system = [
    ...
    '6. WRITES — when a story asks the user to ADD, SAVE, SUBMIT, CREATE, RECORD, LOG, TRACK,',
    '   REQUEST, REGISTER or otherwise CAPTURE information that is NOT already in a granted dataset,',
    '   you MUST include a WRITE tab: the "form" pattern (or "intake-wizard" for multi-step) with',
    '   config target:"records". This persists into the app\'s OWN governed record store — that store',
    '   IS the app\'s table for the new data; you do NOT need, and cannot create, a dataset for it.',
    '   Pair each write with a read tab ("records-table", "approval-queue" or "task-checklist") whose',
    '   config uses source:"records", so the saved rows are visible. NEVER ship a read-only app when',
    '   the stories clearly require data entry — a missing write tab means the epic cannot be fulfilled.',
    '',
    'AppSpec shape:',
