...
- `get_dataset(datasetId)` — medallion versions, docs, data-quality rules, AND `cube.ready`/`cube.view`: a shared/certified dataset with a built Gold is AUTO-REGISTERED as a queryable Cube model (dimensions from the gold columns, `count` fallback) — no define_metric needed.
...
**Cube auto-registration:** publishing a Gold dataset (promote → shared/certified with Gold built) AUTOMATICALLY registers it as a Cube model in `/api/cube/models`. You do NOT run define_metric to get a queryable semantic model — measures are additive.
