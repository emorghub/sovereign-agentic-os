...
**Purpose:** Capture business processes (also called: workflows) as structured knowledge, index them, and
...
- `author_knowledge(title, domain?, markdown?, steps[], rules[], tacit?)` — create
  a Personal draft business process. Steps accept `{title, actor, actor_name, inputs,
  outputs, tacit}`. The top-level `tacit` string is the process-level TACIT.md
...
- `index_knowledge(workflowId)` — chunk → embed → index; required before
  `search_knowledge` returns the business process. Both per-step tacit notes and the
  process-level tacit doc are indexed as separate `type: "tacit"` units.
...
- `retire_knowledge(workflowId, action?)` — `archive` (default, reversible) or
  ...
  a Domain business process). LINEAGE-AWARE: blocked (409) if any App/Agent still consumes
  it. `delete` also refuses a still-live business process.
...
**In-app knowledge agent:** captures a business process as structured steps + rules + tacit.
Three sections: "1. The business process, step by step", "2. Rules and decisions",
"3. Tacit business context".

**Golden path:** author business process (steps + rules + tacit) → index → `search_knowledge`
