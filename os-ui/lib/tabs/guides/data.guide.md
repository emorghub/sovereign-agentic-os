...
1. **Reuse check.** Call `list_datasets` (no arguments — it returns everything you can see, grouped by tier) and scan for your domain and concept. If the dataset already exists, call `get_dataset` and add a version rather than creating a duplicate. Call `query_data` to inspect existing rows before any write.
2. **Create.** Call `create_dataset` with `name` and (optionally) `domain` plus seed `columns` docs. This creates a My-scope dataset in your domain (yours, no approval needed); the Bronze→Silver→Gold versions are added next.
3. **Add a Bronze version.** Two ways:
   ...
   - **Registry-only:** call `add_dataset_version` with `datasetId` and `layer: "bronze"`. Bronze is append-only; no transformations required.
...
5. **Promote to Silver.** Two ways:
   ...
   - **Authored:** call `add_dataset_version` with `datasetId` and `layer: "silver"`, supplying your authored dbt SQL (+ tests) as `body`. Include `not_null` and `unique` tests in the SQL body. Silver requires at least one passing test.
6. **Promote to Gold.** Two ways:
   ...
   - **Authored:** call `add_dataset_version` with `datasetId` and `layer: "gold"`. Gold locks the schema — downstream metric definitions depend on it.
