...
export const warehouseTools: McpTool[] = [
  ...
  },
  {
    ...
    minRole: 'creator',
    description:
      'IMPORT one federated external table into the OS Iceberg lakehouse as an owned data product — a governed CTAS (CREATE TABLE iceberg.<domain>.<name> AS SELECT * FROM <catalog>.<schema>.<table>) run through the SAME promote/materialize path (Trino→OPA as you). NOTE: the GOVERNED lakehouse path is now expose → adopt — a platform admin runs create_exposure_set to grant tables to domains, then a domain admin runs adopt_exposed_table (list_exposed_tables) to create a live-federated or synced dataset with drift/revocation honesty. Prefer that for shared data; this one-shot CTAS materialize stays for a personal/developer copy. This is distinct from marketplace import_product (a listing GRANT). Before: create_connection (warehouse) + register the catalog + test_connection so the catalog is queryable. After: the imported table is a normal sovereign dataset. Governance: requires edit rights on the connection; the query-tool re-validates the CTAS allowlist + target-schema/role gate before Trino runs it.',
    inputSchema: {
      type: 'object',
      properties: {
        connId: { type: 'string', description: 'Warehouse connection id from list_connections.' },
