...
test('TAB-SET Plan group is Strategy, Big Bets, Operating Model, Business Workflows, Marketplace', () => {
  assert.deepEqual(
    PLAN_GROUP.tabs.map((t) => t.label),
    ['Strategy', 'Big Bets', 'Operating Model', 'Business Workflows', 'Marketplace'],
  );
});
