...
import { listConnectionsForUser, getConnectionForUser } from '@/lib/experimental/connections/store';
import {
  type OmConn,
  type OmRead,
  type OmWrite,
...
import {
  ...
  previewDqSync,
  applyDqSync,
} from '@/lib/experimental/connections/openmetadata-dq';
import type { CheckResult } from '@/lib/experimental/data/dq';
import {
  type OmSyncPlan,
  type OmSyncPreview,
  type OmSyncResult,
  ...
  softDeleteOmDataset,
  reactivateOmDataset,
} from '@/lib/experimental/connections/openmetadata-sync';
import type { Dataset } from '@/lib/experimental/data';
import type { CatalogAsset, SourceSeverity } from '@/lib/experimental/data/catalog';
...
export async function omDqAppenderFor(
  ...
  if (!c) return null;
  const write = omWriteConnFrom(c);
  const { osDqTestCaseFqnsForRule } = await import('@/lib/experimental/connections/openmetadata-dq');
