...
} from '@/lib/experimental/connections/schema';
...
import {
  ...
  airflowDagAllowed,
} from '@/lib/experimental/connections/airflow';
import {
  githubConnFrom,
  githubHealth,
...
} from '@/lib/experimental/connections/github';
import {
  supabaseConnFrom,
  supabaseHealth,
...
} from '@/lib/experimental/connections/supabase';
import {
  atlassianConnFrom,
  atlassianHealth,
...
} from '@/lib/experimental/connections/slack';
import {
  googleMailConnFrom,
  gmailHealth,
  gmailListMessages,
  ...
  gmailSendMessage,
  gmailCreateDraft,
} from '@/lib/experimental/connections/gmail';
import {
  gcalConnFrom,
  gcalHealth,
  gcalListCalendars,
  ...
  gcalCreateEvent,
  gcalUpdateEvent,
} from '@/lib/experimental/connections/gcal';
import {
  graphConnFrom,
  outlookHealth,
  outlookListMessages,
  ...
  outlookSendMail,
  outlookCreateDraft,
} from '@/lib/experimental/connections/outlook';
import {
  teamsConnFrom,
  teamsHealth,
  teamsListTeams,
...
} from '@/lib/experimental/connections/teams';
import {
  entraConnFrom,
  entraHealth,
  entraListUsers,
...
} from '@/lib/experimental/connections/entra';
import {
  purviewConnFrom,
  purviewHealth,
  purviewSearchAssets,
  ...
  purviewGetLineage,
} from '@/lib/experimental/connections/purview';
import {
  aiFoundryConnFrom,
  aiFoundryHealth,
  aiFoundryListModels,
...
} from '@/lib/experimental/connections/ai-foundry';
import {
  sagemakerConnFrom,
  sagemakerHealth,
  sagemakerListModels,
...
} from '@/lib/experimental/connections/sagemaker';
import {
  gcpIdentityConnFrom,
  gcpIdentityHealth,
  gcpListProjects,
...
} from '@/lib/experimental/connections/gcp-identity';
import {
  ...
  gcpDirListRoles,
  gcpDirListDomains,
} from '@/lib/experimental/connections/gcp-directory';
import {
...
} from '@/lib/experimental/connections/snowflake-governance';
import type { WarehousePlatform } from '@/lib/experimental/connections/warehouse/types';
import { splitWarehouseFields, toWarehouseSource } from '@/lib/experimental/connections/warehouse/connection';
import { providerFor } from '@/lib/experimental/connections/warehouse/registry';
import { buildImportCtas } from '@/lib/experimental/connections/warehouse/import';
import { catalogRegistration, type CatalogRegistration } from '@/lib/experimental/connections/warehouse/registration';
import { applyLiveRegistration, type RegK8s, type RegisterK8sOutcome, type SecretValues } from '@/lib/experimental/connections/warehouse/k8s-registration';
...
import { createDataset, buildVersion, deleteDataset } from '@/lib/experimental/data';
import { personalSchema, slug } from '@/lib/experimental/data/store-fqn';
import { stageArtifact } from '@/lib/experimental/data/panels';
...
} from '@/lib/experimental/connections/connections-physical-delete';
import {
...
} from '@/lib/experimental/connections/salesforce-tools';
import { allActiveExposures } from '@/lib/experimental/connections/exposures';
import { allActiveAdoptions } from '@/lib/experimental/connections/action-adoptions';
import { executeODataTool } from '@/lib/experimental/connections/odata/client';
import { executeWorkdayTool } from '@/lib/experimental/connections/workday-raas';
import { adapterFor } from '@/lib/experimental/connections/connection-adapters';
import {
  buildPreview,
  matchStandingPolicy,
  rememberPolicy,
...
import { registerBronzeSource, indexToFiles } from '@/lib/experimental/data/data-handoff';
import { logEgress } from '@/lib/experimental/connections/egress-requests';
...
async function recompileExposuresAfterMove(): Promise<void> {
  try {
    const { recompileExposures } = await import('@/lib/experimental/connections/exposure-policy');
...
async function recompileAfterArchive(connId: string): Promise<void> {
  try {
    const [{ allActiveExposures }, { exposureFqns, recompileExposures }] = await Promise.all([
      import('@/lib/experimental/connections/exposures'),
      import('@/lib/experimental/connections/exposure-policy'),
    ]);
...
export function deleteConnectionSync(connId: string, user: CurrentUser): Connection {
  ...
  void import('@/lib/experimental/connections/exposure-propagation')
...
export async function deleteConnection(connId: string, user: CurrentUser): Promise<PhysicalDeleteReport> {
  ...
  const { teardownConnection } = await import('@/lib/experimental/connections/exposure-propagation');
