...
import type { Connection } from '@/lib/experimental/connections/schema';
import { osMirror } from '@/lib/infra/os-mirror';
import { queryRun } from '@/lib/infra/governed';
import { discoverWarehouse, getConnectionForUser } from '@/lib/experimental/connections/store';
import { providerFor } from '@/lib/experimental/connections/warehouse/registry';
import { toWarehouseSource } from '@/lib/experimental/connections/warehouse/connection';
import { operationalEntry } from '@/lib/experimental/connections/operational-registry';
...
export async function describeEntity(
  ...
  if (op.platform === 'salesforce') {
    const { describeSalesforceObject } = await import('@/lib/experimental/connections/salesforce');
  ...
  if (op.platform === 'odata') {
    ...
    const { describeODataEntity } = await import('@/lib/experimental/connections/odata/client');
  ...
  if (op.platform === 'workday') {
    ...
    const { describeWorkdayReport } = await import('@/lib/experimental/connections/workday-raas');
...
export async function countEntity(
  ...
  const op = operationalEntry(c.template);
  if (op?.platform === 'salesforce') {
    const { countSalesforceObject } = await import('@/lib/experimental/connections/salesforce');
  ...
  if (op?.platform === 'odata') {
    ...
    const { countODataEntity } = await import('@/lib/experimental/connections/odata/client');
