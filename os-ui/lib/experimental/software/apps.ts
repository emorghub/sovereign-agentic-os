...
import { emptyAgentGrants, normalizeAgentGrants, type AppAgentGrant } from '@/lib/experimental/software/app-agent-grants';
...
import type {
  ...
  ScaffoldFile,
  SurfaceDeclaration,
} from '@/lib/experimental/software/model';
import { normalizeSpec, type StorySpec } from '@/lib/experimental/software/story-spec';
import type { AppSpec, SpecIssue } from '@/lib/experimental/software/appspec/schema';
import { parseAppSpec } from '@/lib/experimental/software/appspec/schema';
import type { SpecWarning } from '@/lib/experimental/software/appspec/validate';
import { detectPreviewShape } from '@/lib/experimental/software/preview-shape';
import { viteOsFiles } from '@/lib/experimental/software/scaffolds/vite-os';
import { sovereignAppFiles, sovereignAppGuide } from '@/lib/experimental/software/scaffolds/sovereign-app';
import { websiteFiles, websiteGuide } from '@/lib/experimental/software/scaffolds/website';
import { apiServiceFiles, apiServiceGuide } from '@/lib/experimental/software/scaffolds/api-service';
import { emptyAppFiles, emptyAppGuide } from '@/lib/experimental/software/scaffolds/empty-app';
import { vendorSdkForRepo, applySdkFileDep } from '@/lib/experimental/software/app-sdk-vendor';
import { vendorUiForRepo, applyUiFileDep } from '@/lib/experimental/software/app-ui-vendor';
import { snapshotFiles, getSnapshot, hydrateSnapshot, deleteSnapshot } from '@/lib/experimental/software/snapshot';
import { generateAndCompile } from '@/lib/experimental/software/auto-mcp';
import { dataPlaneToolsFromGrants, agentToolsFromGrants } from '@/lib/experimental/software/grant-tools';
import { parseAppManifest, renderAppYaml, defaultOpenApi, resolveSurface } from '@/lib/experimental/software/metadata';
...
export async function setAppSpec(
  ...
  const { summarizeSpecChange, autoVersionName } = await import('@/lib/experimental/software/appspec/change-summary');
...
function triggerAutoRepair(app: App, failedRun: { id: string; headSha: string }): void {
  ...
  void import('@/lib/experimental/software/ci-repair')
 ...
 * `harborRegistry` is `<host>/<owner>` (e.g. `forgejo-http:3000/gitea_admin`).
 */
function buildRuntimeFromConfig(): import('@/lib/experimental/software/build-service').BuildRuntime {
...
export async function submitOsBuild(
  app: App,
  sha: string,
  client?: import('@/lib/experimental/software/build-service').K8sClient,
): Promise<{ submitted: boolean; ok: boolean; detail: string }> {
  const m = await import('@/lib/experimental/software/build-service');
...
export async function refreshBuildStage(
  ...
  if (st.phase === 'succeeded' && st.imageDigest) {
    ...
    try {
      ...
      const review = await import('@/lib/experimental/software/review');
