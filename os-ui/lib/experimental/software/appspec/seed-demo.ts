...
import type { AppSpec } from '@/lib/experimental/software/appspec/schema';
...
export async function seedDeclarativeDemoApp(): Promise<void> {
  try {
    ...
    const { peekDatasetMeta, getDataset, ensureHydrated: ensureDataHydrated } = await import('@/lib/experimental/data/store');
    const {
      createApp,
    ...
    } = await import('@/lib/experimental/software/apps');
