...
3.5e ✅ **IN-PLACE-EDIT patterns** (`editable-grid`, `kanban-workflow`, `action-detail`) — the previously-deferred trio, now real. The store stays **APPEND-ONLY** (durable + auditable): `os.records.update(id, record)` appends a record that SUPERSEDES a logical row (`{ ..., _key:id }`) and `os.records.remove(id)` appends a reversible tombstone (`{ _key:id, _deleted:true }`) — BOTH go through the same governed `add` door (no new route, gate, or store verb). Current rows are DERIVED by `reduceByKey(recordsFromList(await os.records.list()))` (records-reduce.ts): latest append per `_key` wins, tombstones hidden, creation-order stable, reserved keys stripped, each row's `id` IS its logical key. Renderers `EditableGridRenderer` (inline-edit table: add/update/remove), `KanbanWorkflowRenderer` (move a card between status columns → update the status field), `ActionDetailRenderer` (record + governed action buttons that set a field). All use `isRealSave` labelling + surface `Forbidden` verbatim; success is never faked. All three are authorable-by-selection in COMPOSE (bespoke editors) and offered to the generator/assistant. The append log gives free history/audit/undo — the reduce-the-log philosophy, extended from single-key derivations (decisions, done-flags) to full keyed upsert.
