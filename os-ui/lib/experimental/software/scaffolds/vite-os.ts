...
function srcAppTsx(name: string): ScaffoldFile {
  return {
    ...
    content: [
      ...
      "import type { OsContext, QueryResult, WhoAmI } from '@sovereign-os/app-sdk';",
      ...
      '  // os.metrics.query resolves to a QueryResult ({ columns, rows, rowCount }); we JSON-preview it.',
      '  const [sample, setSample] = useState<{ metricName: string; result: QueryResult } | null>(null);',
...
function srcOsTs(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      ' */',
      'export function createOsClient(appSlug?: string) {',
      ...
      '  // appSlug is REQUIRED only for os.records.* (the app\'s own records routes are',
      '  // keyed by the frozen slug). Pass APP_SLUG from core/store.ts; reads never need it.',
      '  return _create({ baseUrl: base, appSlug });',
      '}',
      '',
    ].join('\n'),
  };
...
function nginxConf(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      '    # The try_files internal-redirect lands in the `= /index.html` location below,',
      '    # which is where the no-cache header for the app shell is applied.',
      '    location / {',
      '        try_files $uri $uri/ /index.html;',
      '    }',
      '',
      '    # CACHE POLICY (deploy-safety): the HTML shell must NEVER be heuristically',
      '    # cached, or browsers keep running an OLD bundle after every deploy. The Vite',
      '    # build emits content-hashed files under /assets/ (name changes on every',
      '    # build), so those are safe to cache forever (immutable).',
      '    location = /index.html {',
