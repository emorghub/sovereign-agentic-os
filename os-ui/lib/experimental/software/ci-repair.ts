...
import { diffTrees } from '@/lib/experimental/software/build-changeset';
...
export type { CiRepairState } from '@/lib/experimental/software/apps';
...
async function runScheduledCheck(appId: string): Promise<void> {
  try {
    ...
    const { refreshActionsStage } = await import('@/lib/experimental/software/apps');
