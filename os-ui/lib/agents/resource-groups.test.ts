 ...
 * Bets · Operating Model · Business Workflows) and Context (Knowledge · Files · Data ·
 * Connections · Metrics), with Business Workflows a SEPARATE member from Knowledge.
 */

test('Plan Items are Strategy · Big Bets · Operating Model · Business Workflows, in order', () => {
  const labels = membersOf('plan').map((m) => m.label);
  assert.deepEqual(labels, ['Strategy', 'Big Bets', 'Operating Model', 'Business Workflows']);
});
...
test('Business Workflows is its OWN member, separate from Knowledge', () => {
  ...
  assert.equal(workflows.label, 'Business Workflows');
