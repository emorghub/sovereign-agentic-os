#!/usr/bin/env bash
# Sync os-ui 0.6.98 to the PUBLIC repo (github.com/Data-Masterclass/sovereign-agentic-os).
# BUNDLES 0.6.90→0.6.98. Method: one clean public-line commit whose TREE == private HEAD
...
PUBLIC_TIP=bdd5092          # current public main (0.6.96)
MSG="release: sync to os-ui 0.6.98 — OS error boundaries, app data-access correctness (os.records auto-RW, cause-aware dataset denials, granted-only refs guard), graceful artifact-lifecycle spine (dependency warn-then-allow + SourceUnavailable degradation)"
