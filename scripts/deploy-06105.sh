#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.105 — Software builder stage restructure.
#   0.6.104 metric tiles show clean name (not the domain-qualified Cube member)  [bundled — same image]
#   0.6.105 Software stages renamed + reordered: Define App · Design Epics · Create Context (NEW) · Build App · Test & Publish
#           - Create Context hosts bind-existing grants + create-new datasets (moved out of Define/Design)
#           - Build App gated on resolved context; Test & Publish gated on ≥1 story actually built (real code)
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.105
...
echo "==> helm upgrade (pinned to 0.6.105@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.105@$DIGEST"
