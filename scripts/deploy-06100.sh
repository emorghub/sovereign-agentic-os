#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.100 — CLEARS the baked dev-default-secret error (guard moved to instrumentation.register) + 0.6.99 Science Design + 0.6.100 Science launch.
...
#   0.6.100 Software runs the reasoning model on ALL stages incl. Build
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.100
...
echo "==> helm upgrade (pinned to 0.6.100@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.100@$DIGEST"
