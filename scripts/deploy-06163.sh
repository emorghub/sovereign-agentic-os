...
# One-shot guarded deploy of os-ui 0.6.163 — connectors marked "under construction" (honest preview):
#  The external connectors are not yet end-to-end tested, so the OS now says so everywhere they surface.
#  UI: each Connections gallery tile badge changed teal "ready" -> amber "under construction" + a
#  gallery-wide banner. MCP: list_connection_templates + create_connection open with an UNDER
#  CONSTRUCTION (PREVIEW) note (tell the user connectors are untested, don't rely on them in prod).
#  Docs: OS Guide Connections section + connections.guide.md + Connections tutorial carry the caveat;
#  PDF regenerated. Creating/testing a connector is still allowed (honesty label, not a hard gate).
#  Carries all of 0.6.162.

set -euo pipefail
cd "$(dirname "$0")/.."
export KUBECONFIG="${KUBECONFIG:-deploy/kubeconfig.yaml}"
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.163
...
echo "==> helm upgrade (pinned to 0.6.163@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.163@$DIGEST" \
