#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.107 — Software Build runs on the REASONING model
#   (code-generation was silently on the standard tier; root-cause fix) + 0.6.106 Choose Context rename.
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.107
...
echo "==> helm upgrade (pinned to 0.6.107@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.107@$DIGEST"
