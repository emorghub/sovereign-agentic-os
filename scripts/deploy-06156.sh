...
#     all code-app-era integration actions that no longer fit the DECLARATIVE apps model. Removed the
#     panel + its now-orphaned `hasStories` local. Backend routes left dormant (no UI surface).
#  Carries all of 0.6.155. tsc clean; 955 Software tests green.
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.156
...
echo "==> helm upgrade (pinned to 0.6.156@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.156@$DIGEST"
