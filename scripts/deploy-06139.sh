#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.139 — teach the Software build agent the vendored-app
# API (only-3-import-sources, no react-router, pages return <Section> not <AppShell>, exact
# @sovereign-os/ui + app-sdk signatures) + roles via useIdentity/roleAtLeast (not exact 'admin',
# advisory hide/disable) + done=status+files (no false "already implemented") + self-correction.
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.139
...
echo "==> helm upgrade (pinned to 0.6.139@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.139@$DIGEST"
