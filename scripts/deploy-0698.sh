#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.98 — error boundaries + 0.6.97 (app data-access) + 0.6.98 (graceful lifecycle spine).
...
#   0.6.98 Software runs the reasoning model on ALL stages incl. Build
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.98
...
echo "==> helm upgrade (pinned to 0.6.98@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.98@$DIGEST"
