#!/usr/bin/env bash
# One-shot guarded deploy of os-ui 0.6.112 — Science task auto-detect (0.6.111) + build LLM
# timeout 90s->240s + Choose Context creates app-readable Domain datasets + build prompt hardening.
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.112
...
echo "==> helm upgrade (pinned to 0.6.112@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.112@$DIGEST"
