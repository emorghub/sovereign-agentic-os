...
# (2) NEW platform gate `autonomousAgentsEnabled` (default OFF) — cron/event triggers refused
# fail-closed until a platform admin turns it on in Admin → Platform Settings (manual Run
# unaffected); (3) carries 0.6.151 dataset-id-in-title. Tests: run-graph + scheduled + settings green.
...
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.152
...
echo "==> helm upgrade (pinned to 0.6.152@$DIGEST)"
...
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  ...
  --set osUI.image.tag="0.6.152@$DIGEST"
