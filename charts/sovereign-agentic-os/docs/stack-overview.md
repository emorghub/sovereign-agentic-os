...
stack's services together with the optional extension components configured
for the self-contained deployment.
...
your LiteLLM-compatible gateway.

Configure the LiteLLM master API key using the appropriate local or
secret-management configuration for the deployment. Do not commit real
credentials to Git.

Changing model/API configuration is a runtime configuration change — it does
**not** require rebuilding Docker images.
